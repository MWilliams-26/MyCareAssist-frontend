import './Main.css';

function Main() {
  return (
    <main>
      <section className="welcome__section">
        <h1 className="welcome__title">Your personal medical care assistant</h1>
        <p className="welcome__text">
        We are a team of professionals who are ready to help you with your health.
        </p>
      </section>
      <section className="welcome__features">
        <div className="welcome__features-container">
          <div className="welcome__features-doctor"></div>
          <div className="welcome__features-econtacts"></div>
          <div className="welcome__features-calendar"></div>
          <div className="welcome__features-chat"></div>
        </div>
      </section>
    </main>
  )
}

export default Main;